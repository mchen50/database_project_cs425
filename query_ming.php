...
function getBankAccountID($user_id){
    include("connection.php");
...
function getBankAccountInfo($bacc_id){
    include("connection.php");
    $query ="SELECT balance,number FROM b_account WHERE id = ".$bacc_id."";
    $result = $con->query($query);
    $row = $result->fetch_row();
    $con->close();
    return $row;
...
function getBankInfo($bacc_id){
    include("connection.php");
    $query ="SELECT name, routing_number FROM (bank INNER JOIN has_bank ON bank.id = has_bank.bank_id) WHERE b_account_id =".$bacc_id."";
    $result = $con->query($query);
    $row = $result->fetch_row();
    $con->close();
    return $row;
...
function fetchCards($user_id){
    include("connection.php");
    $query = "SELECT company,number,balance FROM (card INNER JOIN has_card ON card.id = has_card.card_id) WHERE user_id =".$user_id."";
    $result = $con->query($query);
    $con->close();
    return $result;
}
...
function getEaccInfo($eacc_id){
    include("connection.php");
    $query = "SELECT points, balance FROM e_account WHERE id = ".$eacc_id."";
    $result = $con->query($query); 
